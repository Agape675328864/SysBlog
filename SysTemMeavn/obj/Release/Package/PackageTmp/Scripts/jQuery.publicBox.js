;(function($) {
	...
	function showPublicBox(obj, options) {
		...
		// 避免提示框 超出浏览器窗口范围
		var ofs = $(obj).offset();
		ofs.left = Math.min( $(window).width() - opts.width - 50 + $(window).scrollLeft(), ofs.left );
	...
	function regPublicBoxCss() {
		var string = publicBoxId+' {'
				...
				+publicBoxId+' .input_btn{cursor:pointer; height: 22px;padding: 1px 6px; color:#FFFDFD; line-height:100%; float:none;font-family: arial, "microsoft yahei";background-image: linear-gradient(to top,#e74649 0,#df3134 100%);border-radius: 3px;border: 0;margin: auto 5px;}';
